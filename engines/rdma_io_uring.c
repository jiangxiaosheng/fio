...
{
	struct rdma_ioring_options *o = data;
...
static struct fio_option options[] = {
	...
	},
	{
		...
		.type	= FIO_OPT_STR_STORE,
		.off1	= offsetof(struct rdma_ioring_options, bindname),
	...
	},
	{
		...
		.type	= FIO_OPT_INT,
		.off1	= offsetof(struct rdma_ioring_options, port),
		.minval	= 1,
		.maxval	= 65535,
	...
	},
	{
		...
		.alias	= "proto",
		.type	= FIO_OPT_STR,
		.off1	= offsetof(struct rdma_ioring_options, verb),
	...
	// 	.off1	= offsetof(struct rdma_ioring_options, registerfiles),
	...
	// 	.type	= FIO_OPT_INT,
	// 	.off1	= offsetof(struct rdma_ioring_options, sqpoll_thread),
	...
	},
	{
		...
		.type = FIO_OPT_INT,
		.off1 = offsetof(struct rdma_ioring_options, ioring_iodepth),
		.help	= "iodepth for io_uring",
		.category	= FIO_OPT_C_ENGINE,
		.group	= FIO_OPT_G_RDMA_IORING,
	},
	{
...
static int fio_rdma_ioring_setup_qp(struct thread_data *td)
{
	struct rdma_ioring_data *rd = td->io_ops_data;
...
static int fio_rdma_ioring_setup_control_msg_buffers(struct thread_data *td)
{
	struct rdma_ioring_data *rd = td->io_ops_data;
...
static int fio_rdma_ioring_prep(struct thread_data *td, struct io_u *io_u)
{
	struct rdma_ioring_data *rd = td->io_ops_data;
	struct rdma_io_u_data *r_io_u_d;
...
static struct io_u *fio_rdma_ioring_event(struct thread_data *td, int event)
{
	struct rdma_ioring_data *rd = td->io_ops_data;
	struct io_u *io_u;
	int i;
	...
	dprint_io_u(io_u, "fio_rdma_ioring_event");

	return io_u;
}

static int fio_rdma_ioring_getevents(struct thread_data *td, unsigned int min,
...
static int fio_rdma_ioring_send(struct thread_data *td, struct io_u **io_us,
			   unsigned int nr)
{
	struct rdma_ioring_data *rd = td->io_ops_data;
	...
	for (i = 0; i < nr; i++) {
		...
		dprint_io_u(io_us[i], "fio_rdma_ioring_send");
	}
...
static int fio_rdma_ioring_recv(struct thread_data *td, struct io_u **io_us,
			   unsigned int nr)
{
	struct rdma_ioring_data *rd = td->io_ops_data;
...
static enum fio_q_status fio_rdma_ioring_queue(struct thread_data *td,
					  struct io_u *io_u)
{
	struct rdma_ioring_data *rd = td->io_ops_data;
	...
	dprint_io_u(io_u, "fio_rdma_ioring_queue");

	return FIO_Q_QUEUED;
}

static void fio_rdma_ioring_queued(struct thread_data *td, struct io_u **io_us,
			      unsigned int nr)
{
	struct rdma_ioring_data *rd = td->io_ops_data;
	struct timespec now;
...
static int fio_rdma_ioring_commit(struct thread_data *td)
{
	struct rdma_ioring_data *rd = td->io_ops_data;
	struct io_u **io_us;
	int ret;
	...
	do {
		/* RDMA_WRITE or RDMA_READ */
		if (rd->is_client)
			ret = fio_rdma_ioring_send(td, io_us, rd->io_u_queued_nr);
		else if (!rd->is_client)
			ret = fio_rdma_ioring_recv(td, io_us, rd->io_u_queued_nr);
		else
...
static int fio_rdma_ioring_connect(struct thread_data *td, struct fio_file *f)
{
	struct rdma_ioring_data *rd = td->io_ops_data;
...
static int fio_rdma_ioring_accept(struct thread_data *td, struct fio_file *f)
{
	struct rdma_ioring_data *rd = td->io_ops_data;
	struct rdma_conn_param conn_param;
	struct ibv_send_wr *bad_wr;
...
static int fio_rdma_ioring_open_file(struct thread_data *td, struct fio_file *f)
{
	if (td_read(td)) {
		return fio_rdma_ioring_accept(td, f);
	}
	else
		return fio_rdma_ioring_connect(td, f);
}

static int fio_rdma_ioring_close_file(struct thread_data *td, struct fio_file *f)
{
	struct rdma_ioring_data *rd = td->io_ops_data;
	struct ibv_send_wr *bad_wr;
	...
	if ((rd->is_client == 1) && ((rd->rdma_protocol == FIO_RDMA_MEM_READ)
		   || (rd->rdma_protocol == FIO_RDMA_MEM_WRITE))) {
		if (ibv_post_send(rd->qp, &rd->sq_wr, &bad_wr) != 0) {
			log_err("fio: ibv_post_send fail: %m\n");
			return 1;
...
static int fio_rdma_ioring_setup_connect(struct thread_data *td, const char *host,
				    unsigned short port)
{
	struct rdma_ioring_data *rd = td->io_ops_data;
	struct rdma_ioring_options *o = td->eo;
	...
	if (fio_rdma_ioring_setup_qp(td) != 0)
		return 1;

	if (fio_rdma_ioring_setup_control_msg_buffers(td) != 0)
		return 1;
...
static int fio_rdma_ioring_setup_listen(struct thread_data *td, short port)
{
	struct rdma_ioring_data *rd = td->io_ops_data;
	struct rdma_ioring_options *o = td->eo;
	struct ibv_recv_wr *bad_wr;
...
static int fio_rdma_ioring_init(struct thread_data *td)
{
	struct rdma_ioring_data *rd = td->io_ops_data;
	struct rdma_ioring_options *o = td->eo;
	...
	if (td_write(td)) {
		...
		p.cq_entries = rd->ioring_iodepth;
		
		ret = io_uring_queue_init_params(rd->ioring_iodepth, &rd->ring, &p);
		if (ret) {
			log_err("fio: io_uring init fail: %m\n");
			return 1;
		}

		ret = open_backend_log(td);
		if (ret) {
			log_err("fio: open backend log fail: %m\n");
			return 1;
		}
	}
	...
	if (td_read(td)) {	/* READ as the server */
		...
		ret = fio_rdma_ioring_setup_listen(td, o->port);
	} else {		/* WRITE as the client */
		rd->is_client = 1;
		ret = fio_rdma_ioring_setup_connect(td, td->o.filename, o->port);
	}
	return ret;
}

static int fio_rdma_ioring_post_init(struct thread_data *td)
{
	unsigned int max_bs;
	int i;
	struct rdma_ioring_data *rd = td->io_ops_data;
...
static int fio_rdma_ioring_setup(struct thread_data *td)
{
	struct rdma_ioring_data *rd;

	if (!td->files_index) {
		add_file(td, td->o.filename ?: "rdma-ioring", 0, 0);
...
struct ioengine_ops ioengine = {
	...
	.setup			= fio_rdma_ioring_setup,
	.init			= fio_rdma_ioring_init,
	.post_init		= fio_rdma_ioring_post_init,
	.prep			= fio_rdma_ioring_prep,
	.queue			= fio_rdma_ioring_queue,
	.commit			= fio_rdma_ioring_commit,
	.getevents		= fio_rdma_ioring_getevents,
	.event			= fio_rdma_ioring_event,
	.cleanup		= fio_rdma_ioring_cleanup,
	.open_file		= fio_rdma_ioring_open_file,
	.close_file		= fio_rdma_ioring_close_file,
	.flags			= FIO_DISKLESSIO | FIO_UNIDIR | FIO_PIPEIO |
					FIO_ASYNCIO_SETS_ISSUE_TIME,
	.options		= options,
	.option_struct_size	= sizeof(struct rdma_ioring_options),
};
